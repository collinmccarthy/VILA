...
for IDX in $(seq 0 $((CHUNKS-1))); do
    ...
    CUDA_VISIBLE_DEVICES=${GPULIST[$GPU_IDX1]},${GPULIST[$GPU_IDX2]} python -m vila.eval.model_vqa_loader \
        ...
        --answers-file runs/eval/$CKPT/MME/${CHUNKS}_${IDX}.jsonl \
        --num-chunks $CHUNKS \
        --chunk-idx $IDX \
        --conv-mode $CONV_MODE &
done
...
output_file=runs/eval/$CKPT/MME/mme.jsonl
...
for IDX in $(seq 0 $((CHUNKS-1))); do
    cat runs/eval/$CKPT/MME/${CHUNKS}_${IDX}.jsonl >> "$output_file"
done


python $MMEDIR/convert_answer_to_mme.py --experiment $output_file

python $MMEDIR/eval_tool/calculation.py --results_dir runs/eval/$CKPT/MME/mme_results
